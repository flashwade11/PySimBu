...
def simulate_sample(
    ...
    sampled_cells_ids = [
        np.random.choice(
            dataset.obs.index[dataset.obs.cell_type == cell_type],
            size=count,
            replace=True,
        )
    ...
    sampled_cells_adata = dataset[sampled_cells_ids]

    sampled_cells_df = sampled_cells_adata.to_df()
    simulated_count_vector = sampled_cells_df.sum(axis=0).to_frame().T

    sampled_cell_type = dataset.obs.loc[sampled_cells_ids, "cell_type"].to_dict()
...
def simulate_bulk(
    ...
    simulation_results = Parallel(n_jobs=n_jobs, verbose=verbose)(
        delayed(simulate_sample)(dataset, sample_vector, total_cells=n_cells)
    ...
    simulation.obs = pd.DataFrame(dict(n_cells=n_cells, label=label), index=simulation.obs.index)
